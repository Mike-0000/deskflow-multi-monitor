...
void DaemonIpcServer::sendStatus(QLocalSocket *clientSocket, const QString &statusJson)
{
  writeToClientSocket(clientSocket, QStringLiteral("status=%1").arg(statusJson));
}
...
void DaemonIpcServer::processCommand(QLocalSocket *clientSocket, const QString &command, const QStringList &parts)
{
  if (command == QStringLiteral("logLevel")) {
    ...
    writeToClientSocket(clientSocket, QStringLiteral("%1=start").arg(kAckMessage));
  } else if (command == QStringLiteral("stop")) {
    LOG_DEBUG("daemon ipc server got stop message");
    Q_EMIT stopProcessRequested();
    writeToClientSocket(clientSocket, QStringLiteral("%1=stop").arg(kAckMessage));
    ...
    writeToClientSocket(clientSocket, QStringLiteral("%1=clearSettings").arg(kAckMessage));
  } else if (command == QStringLiteral("status")) {
    LOG_VERBOSE("daemon ipc server got status request");
    Q_EMIT statusRequested(clientSocket);
  } else {
...
void DaemonIpcServer::processLogLevel(QLocalSocket *clientSocket, const QStringList &messageParts)
{
  ...
  writeToClientSocket(clientSocket, QStringLiteral("%1=logLevel").arg(kAckMessage));
}
...
void DaemonIpcServer::processConfigFile(QLocalSocket *clientSocket, const QStringList &messageParts)
{
  ...
  writeToClientSocket(clientSocket, QStringLiteral("%1=configFile").arg(kAckMessage));
}
